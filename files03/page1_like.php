...
<input type="text" id="ID" placeholder="Enter your sic" required="required"/>
 ...
 <input type="button"  value="Confirm ID" class="btn-login" onclick="send()" />
	...
	jQuery.ajax({
		url:'send_otp_like.php',
		type:'post',
		data:'ID='+ID,
		success:function(result) {
            if(result=='yes'){
			jQuery('#password_error').html('You have already liked!!');
			}
			if(result=='not_exist'){
			    	window.location='page2_like.php'
			}
			if(result=='error'){
			jQuery('#password_error').html('You are not a registered user!!');
			}
