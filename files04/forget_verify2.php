...
if(isset($_SESSION['jassi1']) && $_SESSION['jassi1']=="true"){
...
echo "<div align='center' style='color:red; font-size:20px'>The OTP is sent to <br>".$Email."</div>"; 
?>	
	 <br/>
<div class="form-input">
<input type="text" id="otp" placeholder="Enter OTP" required="required"/>
	 </div>
<span id="password_error" class="field_error"></span>
	 <br/>
 <input type="button"  value="Submit OTP" class="btn-login" onclick="send()" /> 
 </form>
 </div>
<script>
function send(){
...
jQuery.ajax({
	...
	success:function(result) {
         if(result=='yes'){
			...
			if(result=='not_exist'){
				jQuery('#password_error').html('Please enter valid OTP');
			}
			
			if(result=='set'){
				jQuery('#password_error').html('Please enter the OTP');	
			}
		}
	});
}
